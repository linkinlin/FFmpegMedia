...
#define VIDEO_PICTURE_QUEUE_SIZE 3 //视频队列大小
#define SUBPICTURE_QUEUE_SIZE 16   //字幕队列大小
#define SAMPLE_QUEUE_SIZE 9        //音频队列大小
...
//    int GetRindexShown();
public:
    int frame_queue_init(FFmpegPacketQueue* pktq_, int max_size_, int keep_last_);
    void frame_queue_destroy();
    void frame_queue_signal();
    void frame_queue_unref_item(FFmpegFrame* vp);
    FFmpegFrame* frame_queue_peek();
    FFmpegFrame* frame_queue_peek_next();
    FFmpegFrame* frame_queue_peek_last();
    FFmpegFrame* frame_queue_peek_writable();
    FFmpegFrame* frame_queue_peek_readable();
